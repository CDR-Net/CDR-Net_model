...
def main():
    ...
    CKPT = r"/Users/amy/PycharmProjects/labresearch/CDR-Net/best_change_seg_incdec_ema_bs4.pt"
    ...
    test_ds = BrainChangeSliceDataset(
        root=ROOT, slice_axis=slice_axis, patient_ids_keep=test_ids,
    ...
    if isinstance(ckpt, dict) and "ema_state_dict" in ckpt:
        ...
        print("Loaded EMA weights from:", CKPT)
    elif isinstance(ckpt, dict) and "model_state_dict" in ckpt:
        state = ckpt["model_state_dict"]
        print("Loaded model weights (no EMA found) from:", CKPT)
    else:
        state = ckpt
        print("Loaded:", CKPT)
    ...
    report = eval_delta_and_recon(model, test_loader, device, thr_inc=thr_inc, thr_dec=thr_dec)
    ...
    out_path = os.path.join(os.path.dirname(CKPT), "../test_report_ema.json")
